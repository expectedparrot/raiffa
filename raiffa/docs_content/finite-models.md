...
This page describes implemented behavior. the repository successor specification is the broader
...
JSON Schema from `raiffa model schema` is included in distributions.
`model example --output model.json` and
`model example --sequential --output sequential.json` are complete accepted inputs.
